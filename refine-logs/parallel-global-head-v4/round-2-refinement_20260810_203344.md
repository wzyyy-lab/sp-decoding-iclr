# Round-2 Refinement：PARC-16 / Fixed-Reference Gain-Constrained Parallel Correction
...
4. 删除L4 fallback；D256/L2 capacity失败不允许width rescue；
5. 2K降为nonbinding mechanics/方向smoke，25K才是第一个binding efficacy gate；
6. frozen与joint是recipe deletion test。frozen若相当或更好就删joint，而不是判方法失败；
...
4. 删除“joint representation”贡献。joint DFlash optimization 只是 main recipe，必须
   与 frozen-DFlash matched arm 比较；它不能单独支持论文 claim。
...
在训练前的冻结512-record capacity set上，用与production相同inputs同时执行BF16
online path与FP32 reference replay，取所有protected Top1-vs-runner-up margin差的最大
绝对误差`e_num`，冻结`delta_min=2*e_num`；该值只由数值replay决定，不能看EAL或
调参。若`delta_b<=delta_min`，整个block标为ambiguous并令`Hbar_b=1`。launcher必须
先报告其prompt-balanced比例；若比例本身`>1%`，当前surrogate约束先验不可行，停止并
回到方法审查，禁止启动训练。

如果reference token掉出live Top16，`Hbar_b=1`；若`a_b=0`，`Hbar_b=0`。stable
branch只在`delta_b>delta_min`时做除法，因此constraint gradient被`2/delta_min`
显式上界，不会被任意小非零margin放大。

该分段线性（不是smooth）归一化没有手调温度，并有两个直接性质：

1. step-0 identity 时 `M_b=-delta_b`，所以 `Hbar_b=0`；
...
stop-gradient。dual EMA固定`c_0=0`。若`lambda=100`触顶且EMA violation仍`>0`，或
support-drop rate不回落，立即标记`constraint_infeasible`并停止；不能继续训练再靠
checkpoint挑选掩盖。报告三项未加权loss、gradient norm、lambda、support-drop rate、
ambiguous rate、`Mean(Hbar)`和actual harm，不能只报总loss。
...
### M0：数学与单次并行语义（CPU/A40 smoke）

只验证会使结论失真的事项：full16 shape、所有256 nodes global visibility、一次
`[B,16]`输出、step-0逐token identity、`H<=Hbar`枚举、identity `Hbar=0`、moving-base
反例不会缩短protected set、gold掉出live Top16 fail-closed。禁止哈希式形式检查。

### M1：512-block same-set optimization diagnostic

冻结reference；先冻结DFlash，只训练2.438M head。必须同时满足：

- 唯一binding recovery定义为
  `(EAL_PARC-EAL_ref)/(EAL_Top16_oracle-EAL_ref) >=95%`；
- actual harmed-block fraction `<=1%`；
- upper-bound mean `<=1%`；
- 所有 early-error population进入统计，不允许stable-filter替代。

另报告complete-prefix/J2与token accuracy但它们不替代上述EAL recovery。M1先证明
冻结的dual update在synthetic与capacity上能满足约束；`lambda`触顶仍违反即infeasible。

失败只修已证实的实现错误，否则关闭objective。成功不构成泛化证据。

### M2：2K nonbinding smoke，仍只看主目标

现成R047 2K规模已知不适合裁决最终transfer，因此只做一个
`global + fixed-reference objective + joint DFlash` nonbinding smoke。使用既有
`1589 train / 199 select / 199 diagnostic` prompt split，并在原外部147-prompt
`validation_select`上只报告方向。要求：finite、无runtime错误、actual harm`<=1%`、
相对reference EAL点估计为正且任何域不低于reference超过`0.10`。失败先定位实现/优化，
但不能因2K未超过Domino就科学关闭；也不因此运行width/loss grid。

### M3：25K是第一个binding efficacy pilot

按相同full16 authority收集25,000 unique prompts，预先分为20,000 train、2,500
select、2,500 heldout，三域分层且prompt-disjoint。先并行训练两个真正可能部署的
global recipes：

1. global PARC + joint DFlash；
2. global PARC + frozen DFlash。

两者各自只按select EAL且actual harm`<=1%`选checkpoint；再选择select上更好的global
recipe（完全tie优先frozen），并只为该recipe训练一个parameter-matched local control。
heldout在三者冻结后一次打开。

25K binding gate：

- selected global EAL `>=7.55` 且 actual harm `<=1%`；
- selected global相对其matched local `>=0.15 EAL`，paired prompt bootstrap 95% CI
  lower `>0`；
- 三域global均不低于reference；
- support-drop与ambiguous constraint无infeasibility。

若frozen不差于joint，删除joint recipe并以更简单frozen PARC继续；若joint明显胜出才
保留joint。绝不要求joint必须胜frozen。25K gate失败才关闭PARC，不扩width、不调loss、
不做SGLang。constraint deletion只在该主结果成功后作为necessity ablation。

### M4：数据规模，而不是小集延长训练

M3通过后，把已选择且冻结的global recipe扩到现有99K-prompt多样性规模。
最终规模至少匹配旧99,356-prompt global-head证据；若100K仍低于Domino或曲线饱和，
关闭route。只有100K明显继续上升但未到8.325，才允许把同一冻结recipe扩到接近
Domino的1.42M训练多样性；不能改变architecture/loss解释失败。

### M5：binding acceptance and system gates

只有 disjoint fixed EAL达到 `8.3254859086` 才执行 dynamic rollout。必须：
...
- 512 capacity失败：关闭PARC objective；
- 2K smoke方向弱：不作科学关闭，只允许检查实现/固定优化；直接由25K binding gate裁决；
- 25K capacity高但binding transfer失败：关闭，不增加参数；
- 25K global不胜matched local：不能满足用户的global-head要求，关闭global mechanism；
- frozen匹配或胜joint：删除joint并继续frozen PARC，不判方法失败；
- 100K饱和低于Domino：关闭，不用1.42M赌博；
- fixed过门、dynamic失败：同一policy做on-policy full16 data refresh，仍保持一次并行head；
...
| 三臂不识别joint | 删除joint contribution；主臂过门后补matched frozen control |
| Speculative Correction边界遗漏 | 显式引用并把claim收窄到strict one-pass Top16 lossless setting |
| tiny nonzero reference margin | replay-derived `delta_min`；ambiguous fail-closed；比例>1%先验infeasible |
| reference rank after joint update | protected score只用当前live `r_live`，support drop单独fail-closed |
| 2K错误关闭与joint必须胜frozen | 2K nonbinding；25K binding；frozen为recipe deletion winner |
...
已有但弱的global signal集中成真实accepted-prefix gain。M2只排机械/优化故障，M3的
25K disjoint gate才会尽快、且不被已知2K数据不足混淆地证伪它。
