# Round-1 Refinement：PARC-16 / Fixed-Reference Gain-Constrained Parallel Correction
...
5. D-PACE 不再对整段平均优化。新增收益只定义在 reference 已接受长度之后；训练
   梯度是该增量 accepted-length relaxation 的精确一阶梯度。
...
重新发明 soft-summary head。`D256/L2` 精确新增参数为 `2,438,400`，约占537.427M
DFlash 的 `0.454%`；checkpoint 后 derived BF16 table 为 `77,791,232 bytes`。只有
main objective 在 disjoint 数据上同时表现出 train/heldout 欠拟合，才允许一次预注册
`D256/L4 = 3,489,536` 扩容；不能因 transfer failure 扩容。
...
使用 detached coefficient实现 `-G_b/16` 的精确一阶梯度：
...
产品最长16项且在FP32计算。对同一参数 `theta`，上式的梯度正好等于
`grad_theta[-mean_b G_b/16]`。

### Deterministic-harm upper bound

对 reference protected position `i<a_b`，若 reference token仍在 live candidates，
定义

```
m_bi = max_{k: C_bik != y_bi} A_bik - A_bi,r_ref
M_b  = max_{i<a_b} m_bi.
```

部署的 block harm 为 `H_b=1[M_b >= 0]`（用 `>=` 保守覆盖candidate-order tie）。
令 reference 最弱正确 margin为

```
delta_b = min_{i<a_b}(Z_ref,bi0 - max_{k>0} Z_ref,bik),
gamma_b = delta_b / 2.
```
...
如果reference token掉出live Top16，`Hbar_b=1`；若 `a_b=0`，`Hbar_b=0`。
reference exact tie单独标为ambiguous，`Hbar_b=1`，不得用它满足安全门。
...
stop-gradient。报告三项未加权loss、gradient norm、lambda、support-drop rate、
...
- prompt-balanced oracle-gap recovery `>=95%`；
...
### M2：先跑唯一 main arm，直接看 accepted length

在现成 R047 `1589 train / 199 select / 199 heldout` prompt-disjoint full16 split上，
只先运行 `global + fixed-reference objective + joint DFlash`。不用先做一堆次要消融。
固定 checkpoint只按 select EAL且actual harm `<=1%`选择；heldout一次打开。

主臂继续门：heldout EAL至少超过same-job Domino，且相对reference增加 `>=0.75`
EAL、三个域均不低于reference、actual harm `<=1%`。这是便宜的机制screen，不是最终
1.15x claim。若失败，关闭PARC；不扩width、不调loss、不跑SGLang。

只有主臂过门，才在同一split补两个 matched diagnostic：

1. `local + same objective + joint DFlash`，隔离remote full-block信息；
2. `global + same objective + frozen DFlash`，量化joint recipe而不把它写成贡献。

要求global-joint相对两者各 `>=0.15 EAL`且paired prompt bootstrap 95% CI lower `>0`。
constraint deletion只在主结果成功后作为ablation，不在机制未工作时消耗主线GPU。

### M3：数据规模，而不是小集延长训练

M2通过后按相同full16几何收集25K prompts；相同recipe只训练main arm。25K必须达到
EAL `>=7.8`、harm `<=1%`且学习曲线仍正向，才扩到现有99K-prompt多样性规模。
...
### M4：binding acceptance and system gates
...
- capacity高但M2 transfer失败：关闭，不增加参数；
- M2主臂过门但global不胜local：只说明joint/base改进，不能满足用户的global-head要求；
...
已有但弱的global signal集中成真实accepted-prefix gain。M2会尽快证伪它。
