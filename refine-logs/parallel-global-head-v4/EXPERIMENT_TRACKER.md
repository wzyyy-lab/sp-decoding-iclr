...
|--------|-----------|---------|------------------|-------|---------|----------|--------|-------|
| PARC000 | M0 | 本地实现fail-fast（非实验、无GPU） | PARC core/data/loss/train/eval | synthetic unit fixtures only | shape、global visibility、identity、gradient equivalence、`H<=Hbar` | MUST | COMPLETE | 18 focused tests、py_compile、两个bash-n通过；不产生效果结论 |
...
| PARC020 | M1 | 正式full16 train/validation materialization | released pure DFlash + frozen target | train + validation only | full16、8 anchors、reference EAL、storage/coverage | MUST | QUEUED | Job10169014；16-way A800；2026-08-11 PENDING(Priority)；不是smoke |
...
| PARC100 | M2 | 唯一正式科学训练 | global PARC D256/L2 + joint DFlash | 90K train / 5K validation | validation EAL/harm、train losses/dual/support-drop | MUST | QUEUED_DEPENDENCY | Job10169018 afterok:10169014；180K steps；不先跑任何小训练 |
