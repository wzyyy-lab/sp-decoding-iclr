...
**Effective:** 2026-08-10 10:49:20 +0800; amended 2026-08-10 20:51:04 +0800  
...
11. **No small-set efficacy detours.** Capacity fitting, same-set replay, and 512/2K-scale training may not be used as efficacy gates or presented as progress toward the accepted-length claim. Do not submit standalone GPU training smoke jobs. Code-level unit, shape, gradient, and fail-fast checks are allowed only as implementation safeguards inside the real workflow; they are not experiments and cannot delay or replace meaningful-scale training.

12. **Real training with independent validation.** The first scientific training run must use a meaningful prompt-diverse full16 dataset with prompt-disjoint train, validation, and held-out partitions fixed before training. Checkpoints are selected only on validation. Held-out is opened only after selection. DFlash, released Domino, and the new head must be evaluated on exactly the same validation and held-out prompts with the same evaluator. Training-set EAL is diagnostic only and must never be reported as validation evidence.
...
- Is this a meaningful-scale real training run rather than a capacity/smoke efficacy detour?
- Are train/validation/held-out prompt IDs disjoint, with checkpoint selection restricted to validation?
