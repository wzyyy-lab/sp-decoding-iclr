...
- Source：已清洗的270K Open-PerfectBlend reserve（chat/code/math各90K），从原始prompt重新生成full16数据；旧15-position cache不能作为PARC输入、label、reference或效果证据。
- Split before labels：按domain分层并用固定seed对sample ID排序，先冻结240K train candidates、15K validation candidates和15K held-out candidates；各partition只沿自身固定reserve顺序补足exact 90,000 train、5,000 validation和sealed 5,000 held-out eligible prompts。三者零prompt重叠，并继续排除既有development/formal prompts。
...
| M1 | 生成真实full16 train/validation并封存held-out | PARC010 reserve split；PARC020 16-way A800 train/validation materialization；PARC030 train-only numeric certificate | exact 90K/5K、sealed heldout candidates零重叠；full16/8-anchor完整；train ambiguity`<=1%`；validation/held-out未进入证书 | 约110–190 A800 GPU-hours，主要是95K target generation/features | target feature存储约数百GB；按prompt共享并分片写入，任一part配额不足则失败而不发布 |
