...
**Route status**：G0_GO_LOCAL_CPU_IMPLEMENTATION；all GPU/data/falsifier/deployment stages remain closed。  
...
|---|---|---|---|---|---|---|---|---|
| PV2-001 | M0 | freeze contract/source/metric/power schemas | protocol only | none | PROSPECTIVE_V2_CONTRACT + fresh review | MUST | COMPLETE | G0 v3 GO；only local source + CPU/mock opened；review SHA `2b1d40e9...` |
| PV2-002 | M1 | native LoRA zero-init/count/merge tests | mock + released config | analytic | 1,835,008；exact zero；merge parity | MUST | IN_PROGRESS | G0-authorized local implementation/CPU mock only |
| PV2-003 | M1 | D-PACE/frontier/constraint/tie tests | A/B/C/D pure functions | analytic | scalar/grad parity；max equivalence；active-switch rejection | MUST | TODO | CPU only；may proceed under G0 |
...
| PV2-015 | M6 | matched main training | A/B/C/D × seeds 0/1/2 | fit + checkpoint | 12 hashes labeled selected_feasible or diagnostic_T_final | MUST | BLOCKED | 4×3 array only after G6 |
...
| PV2-019 | M8a | selected-checkpoint merge/trace/output audit | released vs D seed0 | checkpoint/small replay | graph/dtype/output exactness receipt | MUST | BLOCKED | G8 first substage；only after C1-EFFICACY PASS + frozen seed0 identity |
| PV2-020 | M8b | restart-level latency TOST | released vs merged D seed0 | fixed latency prompts | 20 restart medians + Student-t TOST | MUST | BLOCKED | only after PV2-019 PASS；both receipts decide deployment claim |
...
1. PV2-002：implement native LoRA in an isolated new module。
2. PV2-003：implement D-PACE/frontier/blockwise feasibility pure functions and unit tests。
3. PV2-004：complete transactional optimizer/statistics tests，then obtain a fresh G1 code review。
