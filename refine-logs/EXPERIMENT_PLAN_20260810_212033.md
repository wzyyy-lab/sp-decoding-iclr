...
- Source：按`parallel-global-head-v4/FULL16_GEOMETRY_AND_RESERVE_AMENDMENT_20260810.md`，从同一已清洗Open-PerfectBlend源和相同旧集排除规则预先构建270K候选reserve；候选池不是训练规模，最终active仍严格为90K/5K/5K。旧15-position cache不能作为PARC输入、label、reference或效果证据。
...
- Train/validation materialization：只接受普通pre-EOS greedy恰好生成完整129-token continuation的prompt，并在预分配的同split/domain reserve中补足精确90K/5K；使用8个固定均匀anchor。非shift pure DFlash统一用`[anchor]+16 masks -> raw17 -> rows1:17`得到16个prediction states；released Domino保留native shift-label raw16。每个anchor保存16个gold、reference Top16/accepted length和训练所需ordinary DFlash inputs。为支持joint DFlash且避免每step重跑4B target，按prompt保存一次BF16 selected-layer context features，8个anchor共享。
...
| M1 | 生成真实full16 train/validation并封存held-out | PARC010 reserve split；PARC020 16-way A800 train/validation materialization；PARC030 train-only numeric certificate | exact 90K/5K/5K零重叠；pre-EOS 129/full16/8-anchor完整；train ambiguity`<=1%`；validation/held-out未进入证书 | 约110–220 A800 GPU-hours，主要是eligibility target generation/features | target feature存储约数百GB；按prompt共享并分片写入，支持任务级恢复 |
